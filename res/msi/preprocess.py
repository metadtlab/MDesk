...
def make_parser():
    ...
    parser.add_argument(
        "--app-name", type=str, default="MDesk", help="The app name."
    )
    parser.add_argument(
...
def replace_app_name_in_custom_actions(app_name):
    ...
    for file_path in chain(custion_actions_dir.glob("*.cpp"), custion_actions_dir.glob("*.h")):
        with open(file_path, "r", encoding="utf-8") as f:
            lines = f.readlines()
        for i, line in enumerate(lines):
            # RustDesk를 app_name으로 교체 (프린터 드라이버 이름 포함)
...
def update_license_file(app_name):
    ...
    license_content = re.sub("MetaDataLab Ltd", app_name, license_content, flags=re.IGNORECASE)
