...
def build_portable(output_folder: str, target: str):
    ...
    import sys
    if target:
        result = subprocess.run(["cargo", "build", "--release", "--target", target], check=False)
    else:
        result = subprocess.run(["cargo", "build", "--release"], check=False)
    if result.returncode != 0:
        print(f"Error: cargo build failed with exit code {result.returncode}")
