...
lazy_static::lazy_static! {
    ...
    pub static ref APP_NAME: RwLock<String> = RwLock::new("MDesk".to_owned());
...
impl Config {
    ...
    pub fn set_options(mut v: HashMap<String, String>) {
        ...
        // "key" 옵션이 빈 문자열이었으면 다시 추가 (사용자가 명시적으로 키를 지웠음을 표시)
        if key_empty && !v.contains_key("key") {
            v.insert("key".to_owned(), "".to_owned());
        }
        let mut config = CONFIG2.write().unwrap();
        if config.options == v {
            return;
...
pub fn apply_product_default_settings() {
    DEFAULT_SETTINGS
        .write()
        .unwrap()
        .entry(keys::OPTION_APPROVE_MODE.to_string())
...
mod tests {
    ...
    fn test_overwrite_settings() {
        ...
        for (value, expected) in [
            ...
            assert_eq!(password_security::approve_mode(), expected);
        }
        assert!(is_option_can_save(
            &OVERWRITE_SETTINGS,
            key,
