...
pub fn core_main() -> Option<Vec<String>> {
    ...
    if args.is_empty() || crate::common::is_empty_uni_link(&args[0]) {
        ...
        if args[0] == "--remove" {
            ...
            return None;
        } else if args[0].starts_with("-p=[") && args[0].ends_with("]") {
            ...
            if args.len() == 2 && !args[0].contains("host=") {
                if crate::platform::is_installed() && is_root() {
                    ...
                    if let Ok(lic) = crate::custom_server::get_custom_server_from_string(&name) {
                        crate::ui_interface::set_option("is-portable".into(), "Y".into());
                        if !lic.host.is_empty() {
                            // 사용자가 명시적으로 키를 설정했는지 확인 (빈 문자열이어도 설정한 것으로 간주)
                            let options = config::Config::get_options();
                            ...
                            let current_api = crate::ui_interface::get_option("api-server");
                            if current_api.is_empty() {
                                crate::ui_interface::set_option("api-server".into(), lic.api);
                            }
                            let current_relay = crate::ui_interface::get_option("relay-server");
                            if current_relay.is_empty() {
                                crate::ui_interface::set_option("relay-server".into(), lic.relay);
                            }
                            if !lic.password.is_empty() {
                                crate::ipc::set_permanent_password(lic.password).ok();
                            }
                        }
                    }
                } else {
...
fn core_main_invoke_new_connection(mut args: std::env::Args) -> Option<Vec<String>> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            arg if arg.starts_with("-p=[") && arg.ends_with("]") => {
                let encoded = arg.chars().skip(4).take(arg.len() - 5).collect::<String>();
                let password = crate::custom_server::decode_password_external(&encoded);
                param_array.push(format!("password={password}"));
            }
            "--relay" => {
