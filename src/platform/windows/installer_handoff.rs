...
pub(super) fn run_cmds(cmds: String, show: bool, tip: &str) -> ResultType<()> {
    ...
    log::info!("run_cmds begin: tip={tip}, show={show}, verified_handoff=true");
    let exit_code = run_elevated_and_wait(&cmd_path, &parameters, show).map_err(|err| {
        log::error!("run_cmds failed to launch: tip={tip}, error={err}");
        err
    })?;
    log::info!("run_cmds finished: tip={tip}, elevated_exit_code={exit_code}");
    if exit_code != 0 {
        bail!(
