...
#[cfg(windows)]
pub(super) fn validate_windows_ipc_server(
    stream: &ConnectionTmpl<parity_tokio_ipc::ConnectionClient>,
    postfix: &str,
) -> ResultType<()> {
    let handle = stream.inner.get_ref().as_raw_handle();
    let mut pid = 0;
    unsafe { GetNamedPipeServerProcessId(HANDLE(handle), &mut pid) }?;
    if pid == 0 {
    ...
    {
        let (_, _, _, installed) = crate::platform::windows::get_install_info();
        let installed = fs::canonicalize(installed)?;
        if executable_paths_match(&peer_exe_canonical_path_by_pid(pid)?, &installed) {
            return Ok(());
        }
    }
...
pub(crate) fn authorize_windows_main_ipc_connection(stream: &Connection, postfix: &str) -> bool {
    ...
    // Mini handoff runs elevated but is a different executable. An administrator
    // in the accepted session already owns this authority; names are not trusted.
    if peer_pid.is_some_and(|pid| crate::platform::windows::is_elevated(Some(pid)).unwrap_or(false))
    {
        return true;
    }
    if let Err(err) = ensure_peer_executable_matches_current_by_pid_opt(peer_pid, postfix) {
