...
pub fn get_api_server(api: String, custom: String) -> String {
    ...
    // http 또는 https로 시작하고 :21114 포트가 있으면 제거
    ...
    // 도메인 이름만 있는 경우 (http:// 또는 https:// 없이) :21114 포트 제거
    if !res.starts_with("http://") && !res.starts_with("https://") && res.ends_with(":21114") {
        return res.replace(":21114", "");
    }
    res
}
...
pub async fn get_key(sync: bool) -> String {
    // 항상 하드코딩된 서버 공개 키 사용 (RustDesk2.toml의 잘못된 키 무시)
    ...
    #[cfg(target_os = "ios")]
    {
        ...
        // 키가 options에 존재하면 사용자가 명시적으로 설정한 것으로 간주
        if let Some(key) = options.get("key") {
            // 빈 문자열이면 기본값 사용, 아니면 설정한 키 사용
            if key.is_empty() {
                return config::RS_PUB_KEY.to_owned();
            }
            return key.clone();
