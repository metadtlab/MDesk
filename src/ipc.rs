...
pub async fn connect(ms_timeout: u64, postfix: &str) -> ResultType<ConnectionTmpl<ConnClient>> {
    ...
    #[cfg(windows)]
    ipc_auth::validate_windows_ipc_server(&stream, postfix)?;
    Ok(stream)
}
...
async fn get_options_(ms_timeout: u64) -> ResultType<HashMap<String, String>> {
    ...
    if let Some(Data::Options(Some(value))) = c.next_timeout(ms_timeout).await? {
        // 옵션을 가져올 때는 파일에 저장하지 않음 (서버 프로세스의 메모리 캐시가 파일을 덮어쓰는 것을 방지)
        // 파일에 저장하려면 명시적으로 set_option을 호출해야 함
        Ok(value)
    } else {
...
pub fn set_option(key: &str, value: &str) {
    ...
    let is_key_option = key == "key";
    if value.is_empty() && !is_key_option {
        options.remove(key);
    } else {
...
mod test {
    ...
    #[test]
    fn portable_ipc_tokens_are_random_and_reject_wrong_lengths() {
        let token = generate_one_time_ipc_token().unwrap();
        assert_eq!(token.len(), IPC_TOKEN_LEN);
        assert!(token.bytes().all(|b| b.is_ascii_hexdigit()));
        assert_ne!(token, generate_one_time_ipc_token().unwrap());
        assert!(constant_time_ipc_token_eq(&token, &token));
        assert!(!constant_time_ipc_token_eq(&token, &token[..token.len() - 1]));
        assert!(!constant_time_ipc_token_eq("", ""));
        assert!(!constant_time_ipc_token_eq(&token, &"z".repeat(IPC_TOKEN_LEN)));
    }

    #[test]
    fn portable_ipc_handshake_requires_token_and_rejects_replay() {
        let runtime = tokio::runtime::Builder::new_current_thread().enable_all().build().unwrap();
        runtime.block_on(async {
            let expected = generate_one_time_ipc_token().unwrap();
            let mut available = Some(expected.clone());
            for (candidate, accepted) in [("z".repeat(IPC_TOKEN_LEN), false), (expected.clone(), true), (expected, false)] {
                let (a, b) = tokio::io::duplex(4096);
                let mut client = ConnectionTmpl::new(a);
                let mut server = ConnectionTmpl::new(b);
                let (left, right) = tokio::join!(
                    portable_service_ipc_handshake_as_client(&mut client, &candidate),
                    portable_service_ipc_handshake_as_server(&mut server, |candidate| {
                        let matched = available.as_deref().is_some_and(|t| constant_time_ipc_token_eq(t, candidate));
                        if matched { available.take(); }
                        matched
                    }),
                );
                assert_eq!(left.is_ok(), accepted);
                assert_eq!(right.is_ok(), accepted);
            }
            let (a, b) = tokio::io::duplex(4096);
            let mut client = ConnectionTmpl::new(a);
            let mut server = ConnectionTmpl::new(b);
            client.send(&Data::DataPortableService(DataPortableService::Ping)).await.unwrap();
            assert!(portable_service_ipc_handshake_as_server(&mut server, |_| panic!("non-token must not authenticate")).await.is_err());
        });
    }
    ...
    fn verify_ffi_enum_data_size() {
        ...
        // MDesk's source-application/direct-transfer fields extend upstream IPC.
