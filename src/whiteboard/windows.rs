...
pub(super) fn create_event_loop() -> ResultType<()> {
    ...
    let mut draw_strokes: Vec<DrawStroke> = Vec::new();
        ...
        *control_flow = ControlFlow::Poll;
        ...
        match event {
            ...
            Event::UserEvent((k, evt)) => match evt {
                ...
                CustomEvent::Draw(stroke) => {
                    draw_strokes.push(stroke);
                }
                CustomEvent::ClearDraw => {
                    draw_strokes.clear();
                }
                CustomEvent::UndoDraw => {
                    draw_strokes.pop();
                }
                CustomEvent::Exit => {
                    *control_flow = ControlFlow::Exit;
