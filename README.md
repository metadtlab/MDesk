...
MDesk와 MDeskMini는 RustDesk 코드를 기반으로 수정한 프로그램이며, 해당 저작물에는
**GNU Affero General Public License v3(AGPLv3)** 조건이 적용됩니다.
상업적 이용·유료 지원은 가능하지만 원저작권 고지, 이용자의 라이선스상 권리와
해당 배포·네트워크 이용 방식에 따른 소스 제공 의무를 유지해야 합니다.

- [LICENCE](LICENCE): 수정하지 않은 AGPL 원문.
- [NOTICE.txt](NOTICE.txt): 원본 프로젝트, 저작권, 라이선스와 무보증 안내.
- [MODIFICATIONS.txt](MODIFICATIONS.txt): 수정 주체, 날짜와 주요 변경 내역.
- [SOURCE_CODE.txt](SOURCE_CODE.txt): 공개 소스 위치와 버전 대응 검증 상태.
- [RELEASE_CHECKLIST.txt](RELEASE_CHECKLIST.txt): 배포 전에 확인할 소스·고지·패키지 항목.

원본 프로젝트는 [RustDesk](https://github.com/rustdesk/rustdesk), 수정 프로젝트의 소스는
[MDesk 저장소](https://github.com/metadtlab/MDesk)에서 확인할 수 있습니다.
현재 참고용 소스 리비전은 `2b8b3d689d9aad2c87d5112ae5ba6c79fd834674`입니다.
이는 **기존 모든 EXE의 완전한 대응 소스라고 검증한 리비전은 아닙니다.**
실제 배포에는 해당 실행 파일을 만든 소스, 필요한 자체 의존성 패치 및 빌드 절차를
확인하고 버전별 소스 다운로드를 실행 파일 다운로드 옆에 제공해야 합니다.

고객에게 파일을 전달할 때는 `LICENCE`, `NOTICE.txt`, `SOURCE_CODE.txt`,
`MODIFICATIONS.txt`를 함께 전달하고, 소스 다운로드와 고지에 쉽게 접근할 수 있도록 합니다.
관련 제3자 구성 요소의 고지와 별도 소스 제공 조건도 유지합니다.
이 문서의 추가만으로 모든 라이선스 의무가 충족되었다고 판단하지 않습니다.

### 현재 고지 제공 상태

MDeskMini의 새 빌드에서는 원격 대기·실행 중 트레이 아이콘을 우클릭하여 **정보**를
열 수 있습니다. 버전, 저작권, 이용·수정·재배포 및 무보증 안내, 내장 AGPL 원문과
소스 저장소 링크를 제공합니다. 정보 창을 닫아도 원격 연결은 종료되지 않습니다.
본체의 **설정 → MDesk 정보**에서는 실제 버전·빌드 날짜, 저작권과 AGPL 안내,
소스 링크 및 관련 API 서버 저장소를 확인할 수 있습니다. **AGPL-3.0 원문**은 앱에
포함된 원문을 열며 인터넷 연결이 필요하지 않습니다. **구성 요소 라이선스**는
Flutter에 등록된 고지를 표시하며 네이티브 의존성 전체 목록을 대신하지 않습니다.
고정된 개발 도구 버전은 표시하지 않고, 빌드·실행 안내는 이 README로 연결합니다.
AGPL 원문은 Flutter 자산에 포함되지만 별도 배포 고지 파일 전체의 자동 동봉,
버전별 완전한 대응 소스 제공 및 실제 배포 경로의 고지 검증은 남아 있습니다.
빌드 폴더에 수동으로 복사한 고지는 다음 빌드에서 없어질 수 있고,
EXE 옆의 고지 파일이 기존 설치 프로그램 내부에 포함되는 것도 아닙니다.
배포 전 [체크리스트](RELEASE_CHECKLIST.txt)를 적용하십시오.
