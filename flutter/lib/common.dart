...
void showToast(String text, {Duration timeout = const Duration(seconds: 3)}) {
  ...
  // "성공" 또는 "Successful" 단어가 포함되어 있으면 짙은 녹색 배경 사용
  final bool isSuccess = text.contains('성공') || text.toLowerCase().contains('successful') || text.toLowerCase().contains('succeeded');
        ...
        child: Align(
            ...
            child: Container(
              decoration: BoxDecoration(
                color: isSuccess ? const Color(0xFF1B5E20) : MyTheme.color(context).toastBg, // 짙은 녹색 (Material Green 900)
                borderRadius: const BorderRadius.all(
                  Radius.circular(20),
                ),
                border: isSuccess ? Border.all(color: const Color(0xFF81C784), width: 1) : null,
                boxShadow: isSuccess ? [
                  BoxShadow(
              ...
              child: Text(
                ...
                style: TextStyle(
                    ...
                    fontWeight: isSuccess ? FontWeight.bold : FontWeight.w300,
                    fontSize: 18,
                    color: isSuccess ? Colors.white : MyTheme.color(context).toastText),
              ),
            )));
  });
...
bool option2bool(String option, String value) {
  bool res;
  if (option == kOptionEnableAudio ||
      option == kOptionEnableCamera ||
      option == kOptionEnableRecordSession) {
    res = value == "Y";
...
// Keep the both-mode choice explicit when the product default is password-only.
...
/// 버전 비교 - newVersion이 currentVersion보다 높으면 true
