...
import 'package:flutter_hbb/desktop/pages/simple_home_page.dart';
...
bool hasAgentIdInFilename() {
  try {
    ...
    // 실행 파일명 확인 (직접 실행 시)
    final filename =
        Platform.resolvedExecutable.split(Platform.isWindows ? '\\' : '/').last;
...
void runMainApp(bool startService) async {
  ...
  Get.put<WhiteboardReceiverController>(WhiteboardReceiverController(),
      permanent: true);

  // Rust에서 설정한 'is-portable' 옵션을 동기(Sync) 방식으로 읽어옴
  // 또는 agentid가 파일명에 있으면 심플 모드로 설정
  bool isSimpleMode = bind.mainGetOptionSync(key: 'is-portable') == 'Y' ||
  ...
  WindowOptions windowOptions = getHiddenTitleBarWindowOptions(
      ...
      size: isSimpleMode ? const Size(300, 650) : null);
  windowManager.waitUntilReadyToShow(windowOptions, () async {
    ...
    windowManager.setTitle(isSimpleMode ? "MDesk" : getWindowName());
    ...
    return RefreshWrapper(builder: (context) {
      return MultiProvider(
        ...
        child: GetMaterialApp(
          ...
          home: isDesktop
              ? (!(hasAgentIdInFilename() ||
                      bind.mainGetOptionSync(key: 'is-portable') == 'Y')
