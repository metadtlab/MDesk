...
static const char * highcontrast_cursor_xpm[] = {
  "18 24 3 1",
  " 	c None",
  ".	c #000000",
  "+	c #00FFFF",
...
void Viewport::setCursor()
{
  ...
  if (alwaysCursor && (cursorType == "highcontrast")) {
    Fl_Pixmap pxm(highcontrast_cursor_xpm);
    cursor = new Fl_RGB_Image(&pxm);
    cursorHotspot.x = cursorHotspot.y = 0;
  } else if (cursorIsBlank && alwaysCursor) {
...
void Viewport::draw(Surface* dst)
{
  ...
  if ((w() == fbW) && (h() == fbH))
    ...
    frameBuffer->drawScaled(dst, 0, 0, fbW, fbH, x(), y(), w(), h());
}

bool Viewport::captureJpeg(std::vector<uint8_t>& jpeg, std::string& error)
{
  jpeg.clear();
  error.clear();
  ...
  try {
    int stride = 0;
    core::Rect rect = frameBuffer->getRect();
    const uint8_t* buffer = frameBuffer->getBuffer(rect, &stride);

    rfb::JpegCompressor compressor;
    compressor.setQualityLevel(8);
    compressor.compress(buffer, stride, rect, frameBuffer->getPF());

    jpeg.assign(compressor.data(), compressor.data() + compressor.length());
    return true;
...
void Viewport::draw()
{
  ...
  if ((w() == fbW) && (h() == fbH))
    ...
    frameBuffer->drawScaled(0, 0, fbW, fbH, x(), y(), w(), h());
}
...
core::Point Viewport::eventToFramebuffer() const
{
  int px = Fl::event_x() - x();
  int py = Fl::event_y() - y();
...
int Viewport::handle(int event)
{
  ...
  case FL_LEAVE:
    ...
    handlePointerEvent(eventToFramebuffer(), 0);
    return 1;
  ...
  //   * https://github.com/fltk/fltk/pull/1083
    ...
    if (event == FL_MOUSEWHEEL) {
      ...
      // release below
      handlePointerEvent(eventToFramebuffer(), buttonMask | wheelMask);
    } 

    handlePointerEvent(eventToFramebuffer(), buttonMask);
    return 1;
...
void Viewport::initContextMenu()
{
  ...
  fltk_menu_add(contextMenu, p_("ContextMenu|", "About &MDesk - Device..."),
